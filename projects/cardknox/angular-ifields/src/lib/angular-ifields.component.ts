...
export class AngularIfieldsComponent implements AfterViewInit, OnChanges, OnInit {
  ...
  private get iframeContentWindow() {
  ...
  ngAfterViewInit(): void {
    ...
    this.messageHandler = new MessageHandler(this.messagePoster, this.options, this.type, this.log.bind(this));
  ...
  clearIfield() {
    this.messagePoster?.clearIfield();
  }
  ...
  private updateOptionsChanges(val: Options, oldVal: Options) {
  ...
  private updateIssuerChanges(currentValue: string) {
    this.messagePoster?.updateIssuer(currentValue);
  }
  private shouldUpdateIssuer() {
    return this.type === CVV_TYPE;
  }
  private updateThreeDSChanges(val: ThreeDS, oldVal: ThreeDS) {
    if (val.enable3DS)
  ...
  private updateAccountChanges(currentValue: Account) {
    const newAccount = transformAccountData(currentValue);
    this.messagePoster?.setAccount(newAccount);
  }

  private shouldUpdateThreeDS(currentValue: ThreeDS) {
